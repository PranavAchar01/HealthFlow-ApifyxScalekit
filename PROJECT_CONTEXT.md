...
This document details recent and planned UI changes for the 911 Dispatch app (`apps/nine11`).

**Completed Changes (Session 1):**
*   Updated app name from "GuestFlow" to "HealthFlow".
*   Replaced top banner buttons.
*   Removed all emojis and em dashes from the UI.
*   Removed the "Preferred Contact" field.

**Planned Changes (Next Session):**
*   Replace preset patient data with 10 realistic records, including a new `Patient` TypeScript type.
*   Implement an "Add Emergency Contact" feature.
*   Make the "+" button on the Contact Info header functional for incomplete patient records.
*   Wire up the Timeline and Notes tabs with realistic 911 transcripts.

The document also includes key architectural notes, such as the entire UI being in a single file, the state management pattern, and API call details. For the full details, please refer to `HANDOFF.md`.

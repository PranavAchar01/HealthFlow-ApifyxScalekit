...
export async function POST(req: NextRequest) {
  ...
  if (encounter.status === "committed") {
    return NextResponse.json({ error: "Encounter already committed" }, { status: 409 });
  }

  // Agent 4: Identity/Auth verification
  createAuditEntry(
    "identity_auth",
    "PHYSICIAN_AUTH",
  ...
  // Approve specified orders (or all non-blocked)
  ...
  // Agent 5: EHR Write (mock)
  const ehrAudit = createAuditEntry(
    "ehr_write",
    "EHR_COMMIT",
  ...
  // Agent 6: Final immutable audit
  const finalAudit = createAuditEntry(
    "audit",
    "IMMUTABLE_CHECKPOINT",
    `ENTIRE.IO AUDIT: Full lifecycle recorded. Input: ${encounter.paramedicName}. Safety: ${encounter.safetyFlags?.length ?? 0} flags. Approval: ${token.name}. Commit: EHR write successful. Checksum chain verified.`,
    token.userId,
    token.name
  );
  ...
  return NextResponse.json({
    ...
    encounter,
    ehrCommit: {
      physicianId: token.userId,
      physicianName: token.name,
      ordersApproved: idsToApprove.length,
      timestamp: encounter.approvedAt,
      auditChecksum: finalAudit.checksum,
    },
  });
}

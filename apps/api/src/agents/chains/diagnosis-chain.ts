...
function createDiagnosisChain() {
  try {
    ...
    return RunnableSequence.from([DIAGNOSIS_PROMPT, model as any, new StringOutputParser()]);
  } catch {
    return null;
  }
}
...
function fallbackDiagnosis(encounter: Encounter): DiagnosisResult {
  ...
  if (chiefComplaint.includes("Stroke") || conditions.some((c) => c.code === "I63.9")) {
    return {
      primary: "Ischemic Stroke (Large Vessel Occlusion)", icdCode: "I63.9", confidence: 0.89,
      differentials: [{ condition: "Hemorrhagic Stroke", probability: 0.08 }, { condition: "TIA", probability: 0.02 }],
      reasoning: "Left-side hemiparesis with acute onset in elderly patient with atrial fibrillation strongly suggests cardioembolic ischemic stroke.",
    };
  }
  if (chiefComplaint.includes("MI") || chiefComplaint.includes("Chest Pain")) {
    return {
      primary: "ST-Elevation Myocardial Infarction (STEMI)", icdCode: "I21.3", confidence: 0.82,
      differentials: [{ condition: "NSTEMI", probability: 0.1 }, { condition: "Pulmonary Embolism", probability: 0.05 }],
      reasoning: "Acute chest pain warrants immediate 12-lead ECG and troponin.",
    };
  }
  return {
    primary: chiefComplaint || "Undetermined", icdCode: "R69", confidence: 0.5,
    differentials: [{ condition: "Multiple etiologies", probability: 0.5 }],
    reasoning: "Insufficient data. Recommend comprehensive workup.",
  };
}

export async function runDiagnosisChain(encounter: Encounter): Promise<DiagnosisResult> {
  const chain = createDiagnosisChain();
  if (!chain || !encounter.structuredData || !encounter.patientContext) return fallbackDiagnosis(encounter);
  ...
  try {
    const result = await chain.invoke({
      ...
      age: encounter.patientContext.age, sex: encounter.patientContext.sex,
    ...
    return {
      primary: parsed.primary || "Undetermined", icdCode: parsed.icdCode || "R69",
      confidence: Math.min(1, Math.max(0, parsed.confidence || 0.5)),
      differentials: parsed.differentials || [], reasoning: parsed.reasoning || "",
    };
  } catch {

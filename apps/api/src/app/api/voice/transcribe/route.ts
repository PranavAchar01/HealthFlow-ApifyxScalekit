...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey) {
    return NextResponse.json({ error: "ElevenLabs API key not configured" }, { status: 500, headers: corsHeaders(origin) });
  }
  ...
  return NextResponse.json(
    { text: data.text ?? "", language_code: data.language_code ?? "eng", language_probability: data.language_probability ?? 1 },
